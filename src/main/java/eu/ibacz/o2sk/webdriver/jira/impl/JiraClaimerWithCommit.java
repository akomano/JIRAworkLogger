 ...
 *
 */
public class JiraClaimerWithCommit implements JiraClaimer {
	
	private WebDriver driver;
	private BasicJiraOperation jiraHandler;
	
	public JiraClaimerWithCommit(WebDriver wd) {
		this.driver = wd;
	...
	public void claimWorkLogOnCurrentTicket(WorkLogDTO workLog) {
        ...
        (new WebDriverWait(driver, 20)).until(new ExpectedCondition<Boolean>() {
            public Boolean apply(WebDriver d) {
                return driver.findElements(By.id("log-work-dialog")).size() == 0;
            }
        });
	...
	public void createSubtaskOfCurrentTicket(String taskSummary) {
		...
		driver.findElement(By.id("components-textarea")).sendKeys("sp");
		
		driver.findElement(By.id("create-issue-submit")).click();
		
		(new WebDriverWait(driver, 20)).until(new ExpectedCondition<Boolean>() {
            public Boolean apply(WebDriver d) {
                return driver.findElements(By.id("create-subtask-dialog")).size() == 0;
            }
        });
	}
}
